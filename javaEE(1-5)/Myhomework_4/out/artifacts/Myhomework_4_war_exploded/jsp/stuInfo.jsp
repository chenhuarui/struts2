...
<%@ page import="homework.entity.StudentEntity" %>
<%@ page import="homework.entity.ClazzEntity" %>
<%--
...
<body onload="onChange()">
        ...
        <form action="queryStudent.action" method="post" class="form">
            ...
            属性
            <s:select list="#request.listName" name="className" id="className"></s:select>
        ...
        <s:else>
            <table border='1' width='600' cellspacing="4" class="table">
                ...
                %>
                    ...
                    <tr>
                        <td><%=studentEntities.get(i).getClassId()%></td>
                        <td><%=studentEntities.get(i).getStudentId()%></td>
                        <td><%=studentEntities.get(i).getStudentName()%></td>
                        <td><%=studentEntities.get(i).getStudentAge()%></td>

                        <td><a href="input_Student.action?student.studentId=<%=studentEntities.get(i).getStudentId()%>">编辑</a>
                            &nbsp;&nbsp;
                            <a href="delete_Student.action?student.studentId=<%=studentEntities.get(i).getStudentId()%>
                                                                &student.classId=<%=studentEntities.get(i).getClassId()%>">删除</a></td>
                    </tr>
                <% }%>
